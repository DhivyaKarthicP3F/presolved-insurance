...
export default function ChannelUpdateForm(props) {
  const {
    id: idProp,
    channel: channelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = channelRecord
      ? { ...initialValues, ...channelRecord }
      : initialValues;
    setAssignTo(cleanValues.assignTo);
    setContactID(cleanValues.contactID);
    setChannelType(cleanValues.channelType);
    setContactAttributes(
      typeof cleanValues.contactAttributes === "string"
        ? cleanValues.contactAttributes
        : JSON.stringify(cleanValues.contactAttributes)
    );
    setNotes(cleanValues.notes);
    setErrors({});
  };
  const [channelRecord, setChannelRecord] = React.useState(channelModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Channel, idProp)
        : channelModelProp;
      setChannelRecord(record);
    };
    queryData();
  }, [idProp, channelModelProp]);
  React.useEffect(resetStateValues, [channelRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Channel.copyOf(channelRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ChannelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Assign to"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || channelModelProp)}

...
export default function EmailMessageUpdateForm(props) {
  const {
    id: idProp,
    emailMessage: emailMessageModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = emailMessageRecord
      ? { ...initialValues, ...emailMessageRecord }
      : initialValues;
    ...
    setAttachments(
      typeof cleanValues.attachments === "string"
        ? cleanValues.attachments
        : JSON.stringify(cleanValues.attachments)
    );
    setReceivedTime(cleanValues.receivedTime);
    setErrors({});
  };
  const [emailMessageRecord, setEmailMessageRecord] = React.useState(
    emailMessageModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(EmailMessage, idProp)
        : emailMessageModelProp;
      setEmailMessageRecord(record);
    };
    queryData();
  }, [idProp, emailMessageModelProp]);
  React.useEffect(resetStateValues, [emailMessageRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            EmailMessage.copyOf(emailMessageRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "EmailMessageUpdateForm")}
      {...rest}
    >
      <TextField
        label="Channel id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || emailMessageModelProp)}

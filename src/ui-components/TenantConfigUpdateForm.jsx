...
export default function TenantConfigUpdateForm(props) {
  const {
    id: idProp,
    tenantConfig: tenantConfigModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = tenantConfigRecord
      ? { ...initialValues, ...tenantConfigRecord }
      : initialValues;
    setName(cleanValues.name);
    setCompany(cleanValues.company);
    setAdminEmail(cleanValues.adminEmail);
    setPhone(cleanValues.phone);
    setIsSignedup(cleanValues.isSignedup);
    setIntents(
      typeof cleanValues.intents === "string"
        ? cleanValues.intents
        : JSON.stringify(cleanValues.intents)
    );
    setTemplates(
      typeof cleanValues.templates === "string"
        ? cleanValues.templates
        : JSON.stringify(cleanValues.templates)
    );
    setChannels(
      typeof cleanValues.Channels === "string"
        ? cleanValues.Channels
        : JSON.stringify(cleanValues.Channels)
    );
    setInstanceURL(cleanValues.instanceURL);
    setConnectInstanceURL(cleanValues.connectInstanceURL);
    setErrors({});
  };
  const [tenantConfigRecord, setTenantConfigRecord] = React.useState(
    tenantConfigModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(TenantConfig, idProp)
        : tenantConfigModelProp;
      setTenantConfigRecord(record);
    };
    queryData();
  }, [idProp, tenantConfigModelProp]);
  React.useEffect(resetStateValues, [tenantConfigRecord]);
  const validations = {
    name: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            TenantConfig.copyOf(tenantConfigRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "TenantConfigUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || tenantConfigModelProp)}
